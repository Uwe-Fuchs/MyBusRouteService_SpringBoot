...
public class BusRouteDataFileUtilTest
  ...
  public void testProcessSingleLine()
    ...
    final Map<Integer, Map<Integer, Integer>> dataMap = BusRouteDataFileUtil.readAndCacheBusRouteData(pathname);
  ...
  public void testWithNonUniqueBusRouteIds()
    ...
    final Map<Integer, Map<Integer, Integer>> dataMap = BusRouteDataFileUtil.readAndCacheBusRouteData(testFile);
  ...
  public void testTooManyStations()
      throws IOException {
    String testLine = this.generateBusRoute(1, DataConstraints.MAX_NUMBER_OF_STATIONS_PER_ROUTE);
    ...
    testLine = this.generateBusRoute(1, DataConstraints.MAX_NUMBER_OF_STATIONS_PER_ROUTE + 1);
  ...
  public void testTooLessStations()
      throws IOException {
    String testLine = this.generateBusRoute(1, DataConstraints.MIN_NUMBER_OF_STATIONS_PER_ROUTE);
    String pathname = this.createTempDataFile(Arrays.asList("1", testLine));
    final Map<Integer, Map<Integer, Integer>> dataMap = BusRouteDataFileUtil.readAndCacheBusRouteData(pathname);
    ...
    testLine = this.generateBusRoute(1, DataConstraints.MIN_NUMBER_OF_STATIONS_PER_ROUTE - 1);
    pathname = this.createTempDataFile(Arrays.asList("1", testLine));
    BusRouteDataFileUtil.readAndCacheBusRouteData(pathname);
  }
  ...
  public void testTooManyBusRoutes()
      throws IOException {
    List<String> testData = this.generateListOfBusRoutes(DataConstraints.MAX_NUMBER_OF_BUS_ROUTES, 100);
    String pathname = this.createTempDataFile(testData);
    final Map<Integer, Map<Integer, Integer>> dataMap = BusRouteDataFileUtil.readAndCacheBusRouteData(pathname);

    assertEquals(DataConstraints.MAX_NUMBER_OF_BUS_ROUTES, dataMap.size());

    testData = this.generateListOfBusRoutes(DataConstraints.MAX_NUMBER_OF_BUS_ROUTES + 1, 100);
